...
public class Snakeladder {
	...
	public static int checkOption(int dice, int position) {
		Random random = new Random();
	...
	public static void playerDiceCount() {
		if (player == PLAYER1) {
			optionPlayerOne = checkOption(dice, playerOnePosition);
			positionCheck(optionPlayerOne, playerOnePosition, dice);
			option = optionPlayerOne;
			dicePlayerOneCount++;
		} else if (player == PLAYER2) {
			optionPlayerTwo = checkOption(dice, playerTwoPosition);
